    ...
    <div className='w-full h-full bg-primary dark:bg-primary-foreground'>
      ...
      >
        ...
        <div className="flex flex-col items-start gap-4 lg:flex-row lg:gap-0">
          ...
          <button className="bg-primary dark:bg-primary-foreground border border-white rounded-full h-12 lg:ml-4 px-3 w-sm flex items-center justify-start hover:cursor-pointer hover:bg-background"
            onClick={
              () => {
                SignUpWithGooglePopUp(
                  () => { window.location.href = constants.routes.home },
                  () => { setError({ primaryMessage: `We've got you on the waitlist`, type: 'success' }) }
                )
              }}>
